...
namespace SharpNeat.EvolutionAlgorithms
{
        ...
        where TGenome : class, IGenome<TGenome>
    {
        ...
        protected override void PerformOneGeneration()
        {
            ...
            //System.IO.File.AppendAllText("profile.txt", System.DateTime.Now + ":startEval" + Environment.NewLine);
            // Evaluate genomes.
            _genomeListEvaluator.Evaluate(_genomeList);
            //System.IO.File.AppendAllText("profile.txt", System.DateTime.Now + ":endEval" + Environment.NewLine);
            ...
            {
                System.IO.File.AppendAllText("final.txt", CurrentGeneration + Environment.NewLine);
                SharpNeat.Genomes.Neat.NeatGenomeXmlIO.SaveComplete((SharpNeat.Genomes.Neat.NeatGenome)(object)_currentBestGenome, true).Save("bestGenome.xml");
            }
        ...
        private SpecieStats[] CalcSpecieStats(out int offspringCount)
        {
            ...
            for(int i=0; i<specieCount; i++)
            {
                ...
                if(i == _bestSpecieIdx && inst._eliteSizeInt==0 && _eaParams.ElitismProportion != 0)
                {
        ...
        private void UpdateStats()
        {
            ...
            /*
            if(_stats._generation % 1000 == 0)
                System.IO.File.AppendAllText("stats.txt", String.Format("Generation: {0}, Average fitness: {1}, Average Complexity: {2}", _stats._generation, _stats._meanFitness, _stats._meanComplexity) + Environment.NewLine);
            */

            
            // if this is a non solvable domain, print off the maximum alt fitness for this generation
            if (!PotEAParams.SOLVABLEDOMAIN)
            {
                double maxAlt = 0;
                double fitForMaxAlt = 0;
                double totalAlt = 0;
                for (int i = 0; i < _specieList.Count; i++)
                {
                    for (int j = 0; j < _specieList[i].GenomeList.Count; j++)
                    {
                        if (_specieList[i].GenomeList[j].EvaluationInfo.AuxFitnessArr[0]._value > maxAlt)
                        {
                            maxAlt = _specieList[i].GenomeList[j].EvaluationInfo.AuxFitnessArr[0]._value;
                            fitForMaxAlt = _specieList[i].GenomeList[j].EvaluationInfo.Fitness;
                        }

                        totalAlt += _specieList[i].GenomeList[j].EvaluationInfo.AuxFitnessArr[0]._value;
                    }
                }
                //System.IO.File.AppendAllText("stats.txt", maxAlt + Environment.NewLine);
                //System.IO.File.AppendAllText("correlation.txt", fitForMaxAlt + "," + maxAlt + Environment.NewLine);

                if(_currentGeneration % 1000 == 0)
                    System.IO.File.AppendAllText("gen.txt", System.DateTime.Now + ":" + _currentGeneration + Environment.NewLine);

                if (SharedParams.SharedParams.TRACKFITNESS)
                {
                    _stats._avgAltFitnessList.Add(totalAlt / _genomeList.Count);
                    _stats._maxAltFitnessList.Add(maxAlt);
                    if (maxAlt > _stats._maxAltFitness)
                        _stats._maxAltFitness = maxAlt;

                    if(_currentGeneration % SharedParams.SharedParams.TRACKFITNESSSTRIDE == 0)
                        System.IO.File.AppendAllText("fitness.txt", maxAlt + Environment.NewLine);
                }

                if (maxAlt > PotEAParams.CHECKPOINTFITNESS && _stats._generationReachedThreshold == 0)
                {
                    _stats._generationReachedThreshold = _currentGeneration;
                    _currentGeneration = (uint)PotEAParams.MAXGENERATIONS; // go ahead and force this run to end.  If we want to track fitness stats past this checkpoint up to max generations, comment this line.
                }
                
                if (_currentGeneration == PotEAParams.MAXGENERATIONS)
                {
                    if (_stats._generationReachedThreshold == 0)
                        _stats._generationReachedThreshold = _currentGeneration;

                    System.IO.File.AppendAllText("final.txt", _stats._generationReachedThreshold + Environment.NewLine);
                    //System.IO.File.AppendAllText("stats.txt", Environment.NewLine + Environment.NewLine);
                    PotEAParams.FINALGENERATION = (int)_stats._generationReachedThreshold;
                    if (SharedParams.SharedParams.TRACKFITNESS)
                    {
                        PotEAParams.FINALMAXFITNESS = _stats._maxAltFitness;
                        PotEAParams.MAXFITNESSLIST = _stats._maxAltFitnessList;
                        PotEAParams.AVGFITNESSLIST = _stats._avgAltFitnessList;
                    }
                }
            }            
        }
