        ...
        where TGenome : class, IGenome<TGenome>
    {
        ...
        public void AlgorithmThreadMethod()
        {
            try
            {
                ...
                for(;;)
                {
                    //System.IO.File.AppendAllText("profile.txt", System.DateTime.Now + ":genstart" + Environment.NewLine);
                    _currentGeneration++;

                    /* profile first generation only
                    if (_currentGeneration == 2)
                        Environment.Exit(0);
                     */
                    // only perform the full evaluation if we're at the fitness stride tracking point
                    ...
                    {
                        if (PotEAParams.SOLVABLEDOMAIN) // only tracking generation # to solve the problem, print out the max since it isn't solved yet
                            System.IO.File.AppendAllText("final.txt", PotEAParams.MAXGENERATIONS + Environment.NewLine);
                        else
                        {
                            //System.IO.File.AppendAllText("stats.txt", Environment.NewLine);
                        }
                        break;
                    }
